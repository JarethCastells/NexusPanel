...
export class MessageService {
  ...
  async saveHistoricMessage(sessionId: string, data: Partial<Message> & { fromMe?: boolean; isGroup?: boolean }): Promise<Message> {
    const waMessageId = data.waMessageId ?? data.id;
    if (waMessageId) {
    ...
    const message = this.messageRepository.create({
      ...
      waMessageId,
      chatId: data.chatId ?? '',
      from: data.from ?? (data.fromMe ? session.phone || 'me' : data.chatId ?? ''),
      to: data.to ?? (data.fromMe ? data.chatId ?? '' : session.phone || 'me'),
      body: data.body,
      type: data.type ?? 'text',
      direction: data.fromMe ? MessageDirection.OUTGOING : MessageDirection.INCOMING,
