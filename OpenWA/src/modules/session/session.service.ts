...
export class SessionService implements OnModuleDestroy, OnModuleInit {
  ...
  private async initializeEngine(id: string, session: Session): Promise<void> {
    ...
    await engine.initialize({
      ...
      onReady: (phone: string, pushName: string): void => {
        ...
        void this.syncHistoricMessages(id, engine);
      },
      onMessage: (message): void => {
        ...
        void this.hookManager
          ...
          .then(async ({ continue: shouldContinue, data: finalMessage }) => {
            ...
            // Persist incoming message in the database so it can be retrieved via API
            try {
              await this.messageService.saveIncomingMessage(id, finalMessage as Partial<Message>);
            } catch (saveError) {
            this.logger.error('Failed to save incoming message', undefined, {
              });
            }
  ...
  async stop(id: string): Promise<Session> {
    ...
    if (engine) {
      try {
        ...
        this.logger.warn('Session logout failed during stop, falling back to disconnect', {
          sessionId: id,
  ...
  private async syncHistoricMessages(id: string, engine: IWhatsAppEngine): Promise<void> {
    if (typeof engine.getRecentChats !== 'function' || typeof engine.fetchChatMessages !== 'function') {
      return;
    }

    this.logger.log('Syncing historic messages for session', {
      sessionId: id,
      action: 'sync_historic_messages',
    });

    try {
      const chats = await engine.getRecentChats(20);
      for (const chat of chats) {
        const messages = await engine.fetchChatMessages(chat.id, 20);
        for (const message of messages) {
          await this.messageService.saveHistoricMessage(id, {
            ...
            metadata: message.media ? { hasMedia: true } : undefined,
          });
        }
      }
    } catch (error: unknown) {
      this.logger.warn('Historic message sync failed', {
        sessionId: id,
        error: error instanceof Error ? error.message : String(error),
      });
    }
