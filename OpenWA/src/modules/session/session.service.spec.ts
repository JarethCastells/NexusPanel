...
describe('SessionService', () => {
  ...
  describe('stop', () => {
    it('should logout engine and set status to DISCONNECTED', async () => {
      const session = createMockSession();
      (repository.findOne as jest.Mock).mockResolvedValue(session);
      (repository.update as jest.Mock).mockResolvedValue({ affected: 1 });
    ...
    it('should fallback to disconnect when logout fails', async () => {
      const session = createMockSession();
      (repository.findOne as jest.Mock).mockResolvedValue(session);
      (repository.update as jest.Mock).mockResolvedValue({ affected: 1 });
      (mockEngine.logout as jest.Mock).mockRejectedValue(new Error('EBUSY: resource busy or locked'));
