...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  async logout(): Promise<void> {
    if (this.client) {
      try {
        ...
        // On Windows, the auth lockfile can be busy/locked briefly while closing.
        // Fallback to destroy to avoid leaving the session in a broken state.
        const errno = (error as NodeJS.ErrnoException)?.code;
        if (errno === 'EBUSY' || errno === 'EPERM' || errno === 'EACCES') {
          this.logger.warn('Logout encountered locked auth file, falling back to destroy', { code: errno });
        }
  ...
  async getRecentChats(limit: number = 20): Promise<{
    id: string;
    name?: string;
    lastMessage?: string;
    timestamp?: number;
    ...
    try {
      ...
      return sorted.map((chat: any) => ({
        ...
        name: chat.name || chat.formattedTitle || undefined,
        lastMessage: chat.lastMessage?.body || chat.lastMessage?.caption || (chat.lastMessage ? `[${chat.lastMessage.type || 'mensaje'}]` : undefined),
        timestamp: chat.lastMessage?.timestamp,
        isGroup: chat.isGroup,
      }));
    } catch (error) {
      this.logger.error('Failed to getChats() from WWebJS', String(error));
      // Try to provide some fallback or rethrow a better message
      throw new Error(`Failed to load chats from WhatsApp: ${String(error)}`);
    }
  }

  async fetchChatMessages(chatId: string, limit: number = 20): Promise<IncomingMessage[]> {
    this.ensureReady();
    try {
