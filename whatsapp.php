        ...
        <div class="wa-shell">
            <section class="wa-side">
                ...
                <div class="wa-section wa-qr" id="qrBox" style="display:none; flex-shrink:0;">
                    ...
                    <p class="wa-muted">Abre WhatsApp → Dispositivos vinculados</p>
                ...
                <div class="wa-convos-wrap">
                    ...
                    <div class="wa-convos-list" id="conversationList"><div class="wa-empty">Selecciona una sesión.</div></div>
                </div>
            </section>
...
function renderSessions() {
    ...
    list.innerHTML = sessions.map(s => `
        <div class="wa-session ${selectedSession?.id === s.id ? 'active' : ''}" data-session-id="${esc(s.id)}">
            ...
            <button class="wa-session-delete" data-delete-id="${esc(s.id)}" title="Eliminar sesión y quitarla del panel"><i class="fa-solid fa-xmark"></i></button>
        </div>
    `).join('');
...
async function loadSessions() {
    try {
        ...
        if (selectedSession) {
            ...
            $('chatSubtitle').textContent = 'Crea o selecciona una sesión.';
            $('statConvos').textContent = '0';
            $('conversationList').innerHTML = '<div class="wa-empty">Selecciona una sesión.</div>';
            renderChat();
        }
    } catch (err) {
...
function startNewChat() {
    if (!selectedSession || !['ready','connected'].includes(String(selectedSession.status || '').toLowerCase())) {
        return alert('Necesitas tener una sesión vinculada y lista para iniciar un chat.');
    }
    const num = prompt('Número de WhatsApp (con código de país, ej. 5215512345678):');
    if (!num) return;
...
async function deleteSession(id) {
    if (!confirm('¿Seguro que deseas ELIMINAR esta sesión por completo? Se removerá del panel.')) return;
    try {
